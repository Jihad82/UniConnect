...
class ExamBusRoutes0 extends StatefulWidget {
  const ExamBusRoutes0({super.key});

  @override
  _ExamBusRoutes0State createState() => _ExamBusRoutes0State();
}

class _ExamBusRoutes0State extends State<ExamBusRoutes0> {
  String localPath = "";
  ...
  Future<void> _preparePdf() async {
    try {
      ...
      DocumentSnapshot snapshot = await FirebaseFirestore.instance.collection('bus_pdfs').doc('exam').get();
      ...
      final File file = File('${tempDir.path}/exam.pdf');
  ...
  Future<void> _downloadPdf() async {
    if (await Permission.storage.request().isGranted) {
      ...
      if (selectedDirectory != null) {
        final String newPath = '$selectedDirectory/exam.pdf';
