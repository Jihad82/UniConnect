...
class ShuttleRoutes1 extends StatefulWidget {
  const ShuttleRoutes1({super.key});

  @override
  _ShuttleRoutes1State createState() => _ShuttleRoutes1State();
}


class _ShuttleRoutes1State extends State<ShuttleRoutes1> {
  String localPath = "";
  ...
  Future<void> _preparePdf() async {
    try {
      ...
      DocumentSnapshot snapshot = await FirebaseFirestore.instance.collection('bus_pdfs').doc('kuril').get();
      ...
      final File file = File('${tempDir.path}/kuril.pdf');
  ...
  Future<void> _downloadPdf() async {
    if (await Permission.storage.request().isGranted) {
      ...
      if (selectedDirectory != null) {
        final String newPath = '$selectedDirectory/kuril.pdf';
