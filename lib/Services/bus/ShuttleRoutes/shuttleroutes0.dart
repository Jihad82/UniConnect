...
class ShuttleRoutes0 extends StatefulWidget {
  const ShuttleRoutes0({super.key});

  @override
  _ShuttleRoutes0State createState() => _ShuttleRoutes0State();
}

class _ShuttleRoutes0State extends State<ShuttleRoutes0> {
  String localPath = "";
  ...
  Future<void> _preparePdf() async {
    try {
      ...
      DocumentSnapshot snapshot = await FirebaseFirestore.instance.collection('bus_pdfs').doc('gausia').get();
      ...
      final File file = File('${tempDir.path}/gausia.pdf');
  ...
  Future<void> _downloadPdf() async {
    if (await Permission.storage.request().isGranted) {
      ...
      if (selectedDirectory != null) {
        final String newPath = '$selectedDirectory/gausia.pdf';
