...
class SpecialBusRoutes0 extends StatefulWidget {
  const SpecialBusRoutes0({super.key});

  @override
  _SpecialBusRoutes0State createState() => _SpecialBusRoutes0State();
}

class _SpecialBusRoutes0State extends State<SpecialBusRoutes0> {
  String localPath = "";
  ...
  Future<void> _preparePdf() async {
    try {
      ...
      DocumentSnapshot snapshot = await FirebaseFirestore.instance.collection('bus_pdfs').doc('special').get();
      ...
      final File file = File('${tempDir.path}/special.pdf');
  ...
  Future<void> _downloadPdf() async {
    if (await Permission.storage.request().isGranted) {
      ...
      if (selectedDirectory != null) {
        final String newPath = '$selectedDirectory/special.pdf';
