...
class Route3Page extends StatefulWidget {
  const Route3Page({super.key});

  @override
  _Route3PageState createState() => _Route3PageState();
}

class _Route3PageState extends State<Route3Page> {
  String localPath = "";
  ...
  Future<void> _preparePdf() async {
    try {
      ...
      DocumentSnapshot snapshot = await FirebaseFirestore.instance.collection('bus_pdfs').doc('route4').get();
      ...
      final File file = File('${tempDir.path}/route4.pdf');
  ...
  Future<void> _downloadPdf() async {
    if (await Permission.storage.request().isGranted) {
      ...
      if (selectedDirectory != null) {
        final String newPath = '$selectedDirectory/route4.pdf';
