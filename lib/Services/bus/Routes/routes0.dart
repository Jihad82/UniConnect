...
class Route0Page extends StatefulWidget {
  const Route0Page({super.key});

  @override
  _Route0PageState createState() => _Route0PageState();
}

class _Route0PageState extends State<Route0Page> {
  String localPath = "";
  ...
  Future<void> _preparePdf() async {
    try {
      ...
      DocumentSnapshot snapshot = await FirebaseFirestore.instance.collection('bus_pdfs').doc('route1').get();
      ...
      final File file = File('${tempDir.path}/route1.pdf');
  ...
  Future<void> _downloadPdf() async {
    if (await Permission.storage.request().isGranted) {
      ...
      if (selectedDirectory != null) {
        final String newPath = '$selectedDirectory/route1.pdf';
