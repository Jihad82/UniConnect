...
class Route10Page extends StatefulWidget {
  const Route10Page({super.key});

  @override
  _Route10PageState createState() => _Route10PageState();
}

class _Route10PageState extends State<Route10Page> {
  String localPath = "";
  ...
  Future<void> _preparePdf() async {
    try {
      ...
      DocumentSnapshot snapshot = await FirebaseFirestore.instance.collection('bus_pdfs').doc('route11').get();
      ...
      final File file = File('${tempDir.path}/route11.pdf');
  ...
  Future<void> _downloadPdf() async {
    if (await Permission.storage.request().isGranted) {
      ...
      if (selectedDirectory != null) {
        final String newPath = '$selectedDirectory/route11.pdf';
