...
class Route11Page extends StatefulWidget {
  const Route11Page({super.key});

  @override
  _Route11PageState createState() => _Route11PageState();
}

class _Route11PageState extends State<Route11Page> {
  String localPath = "";
  ...
  Future<void> _preparePdf() async {
    try {
      ...
      DocumentSnapshot snapshot = await FirebaseFirestore.instance.collection('bus_pdfs').doc('route12').get();
      ...
      final File file = File('${tempDir.path}/route12.pdf');
  ...
  Future<void> _downloadPdf() async {
    if (await Permission.storage.request().isGranted) {
      ...
      if (selectedDirectory != null) {
        final String newPath = '$selectedDirectory/route12.pdf';
